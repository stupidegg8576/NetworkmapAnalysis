        ...
        for conditions in tagyaml[tag]:
            ...
            if 'con' in conditions:
                for subconditions in tagyaml[tag][conditions]:
                    ...
                    tagyaml[tag][conditions][subconditions] = [keyword.replace(
                        keyword, keyword.lower()) for keyword in tagyaml[tag][conditions][subconditions]]
...
def mac_between(target_mac_range: list, mac_addr: str):
    ...
    if len(target_mac_range) < 2:
        ...
        raise BaseException(
            "Tag file error: mac_between() input target mac should a list with 2 mac")
            ...
            if conditions.startswith('con'):
                ...
                for subconditions in tagyaml[tag][conditions]:
                    ...
                    if subconditions.startswith('without'):
                        ...
                        raise BaseException(
                            "Tag file error: can't find subcondition")
...
def apply_tag(device_data: pandas.DataFrame, input_path_tag_vendor_class: str, input_path_tag_host_name: str, max_check: int = 0) -> dict:
    ...
    for device in device_data.iloc():
        ...
        if tags:
            device_with_tag[n] = [
                device['Vendor_Class'], device['Host_Name']] + tags
        else:
            device_without_tag[n] = [
                device['Vendor_Class'], device['Host_Name']]
